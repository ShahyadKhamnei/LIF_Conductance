...
# voltage traces
# in EIF paper, fit parameters of model to data
# expect to depend on steepness of exponential term
# parameters in paper, same nondimensionalization, what do you get for delta
# Or parameterize my model with actual conductances/values
# Step currents
# Fluctuating currents, time varying input, paper from Mainen, experimental paper fluctuating current get reliable spikes on upswings
# Think about deriving action
...
def plot_fig_single_pop_weakly_coupled(savedir=results_dir, Jmax=10, Emax=2):
    ...
    for i in range(len(perturb_starts)):
        E_plot[int(perturb_starts[i]/dt):int((perturb_starts[i]+perturb_lens[i])/dt)] += perturb_amps[i]
    ...
    J = np.zeros((N,N))
    delta = 4
    Vs, spktimes = sim_lif_perturbation_exp(J=J, E_l=E_l, E_s=E_s, tstop=tstop, dt=dt, delta=delta, perturb_starts=perturb_starts, perturb_lens=perturb_lens, perturb_amps=perturb_amps)
    ...
    fig.savefig(os.path.join(savedir, f'sim_perturb_exp_delta{delta}.png'), dpi=600)

    plt.clf()

    fig, axs = plt.subplots(min(5, N), sharex=True, sharey=True)
    for i in range(min(5, N)):
        axs[i].plot(tplot, Vs[:,i], label=f'Neuron {i}')
    
    for ax in fig.get_axes():
        ax.label_outer()
    fig.suptitle(f'Neural Voltage Traces, Delta={delta}')
    axs[min(5, N)-1].set_xlabel('Time (ms/{})'.format(r'$\tau$'), fontsize=fontsize)
    axs[0].set_ylim(0,4)
    fig.savefig(os.path.join(savedir, f'voltage_traces_exp_delta{delta}.png'), dpi=600)
